...
setInterval(() => {
    ...
    const liveCfg = JSON.parse(localStorage.getItem('krypton_pyra_config')) || {};
    const overlay = document.getElementById('maintenanceOverlay');
    if (overlay) {
        if (liveCfg.maintenanceMode === true) {
...
function finishMining(startTime) {
    ...
    if (netProfit > 0) {
        ...
        if (currentUserData && currentUserData.referredBy && AdminAPI.getConfig().squadSystemEnabled !== false) {
...
function renderSquadFullPageContent() {
    ...
    const __squadCfg = JSON.parse(localStorage.getItem('krypton_pyra_config')) || {};
    if (__squadCfg.squadSystemEnabled === false) {
        container.innerHTML = '<div class="no-data-message"><i class="fas fa-users-slash"></i>نظام السكواد متوقف حالياً من قبل الإدارة</div>';
        return;
    }
...
const __maintenanceConfig = JSON.parse(localStorage.getItem('krypton_pyra_config')) || {};
if (__maintenanceConfig.maintenanceMode === true) {
...
console.log('📝 Deposits:', JSON.parse(localStorage.getItem('krypton_deposits')) || []);
console.log('📝 Withdrawals:', JSON.parse(localStorage.getItem('krypton_withdrawals')) || []);
