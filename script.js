...
const EMAILJS_TEMPLATE_ID = '6hlrryp';
...
function sendPlatformEmail(toEmail, toName, subject, message, extraParams = {}) {
    ...
    return emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, templateParams)
        .then((res) => { console.log('✅ تم إرسال الإيميل بنجاح إلى', toEmail); return res; })
        .catch((err) => { console.error('❌ فشل إرسال الإيميل:', err); showToast('⚠️ تعذر إرسال الإيميل، تحقق من الاتصال', 'error'); throw err; });
}
...
function sendOtpToEmail(email) {
    return new Promise((resolve) => {
        ...
        sendPlatformEmail(
            ...
            { otp_code: otp }
        ).catch(() => {}); // حتى لو فشل الإرسال، نكمل التدفق (الرمز محفوظ محلياً كخط دفاع أخير)
